...
前端使用 TypeScript 实现，负责浏览器侧的媒体采集、页面展示、半双工状态控制和实时通信。
...
后端使用 Node.js + TypeScript 实现，负责实时连接、豆包 ASR/TTS 调用、GLM-5V-Turbo 调用、LangGraph 工作流编排和基础用量记录。
